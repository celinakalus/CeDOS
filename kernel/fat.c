...
uint32_t FAT_write(file_t *file, uint8_t *buffer, uint32_t count) {
    uint16_t cluster = file->fat_cluster;
    fpos_t offset = file->pos;
    size_t file_size = file->size;
    uint32_t size = 0;
    ...
    while (count > 0) {
        ...
        memcpy((cluster_buffer + offset), buffer, memcpy_size);

        FAT12_write_cluster(fat_desc, cluster, cluster_buffer);
